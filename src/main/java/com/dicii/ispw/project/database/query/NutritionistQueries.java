...
public class NutritionistQueries extends Queries{
        ...
        public static void updateNutritionistAll(Statement statement,Nutritionist nutritionist) throws SQLException{
            String sql = String.format("UPDATE nutrizionista SET Nome = '%s', Cognome = '%s', DataDiNascita = '%s', Iban = '%s', Iva = '%s', CostoMensile = '%s', Descrizione = '%s' WHERE Email = '%s' ",nutritionist.getName(),nutritionist.getSurname(),nutritionist.getDateOfBirth(),nutritionist.getIban(),nutritionist.getCosto(),nutritionist.getDescription(),nutritionist.getEmail());
